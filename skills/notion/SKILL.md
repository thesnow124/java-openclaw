...
> **注意：** `Notion-Version` 标头是必需的。此技能使用 `2025-09-03`（最新版本）。在此版本中，数据库在 API 中被称为"数据源"。
...
- **搜索结果：** 数据库返回为 `"object": "data_source"` 并带有其 `data_source_id`
...
- 速率限制：平均约 3 个请求/秒
- 创建数据源时使用 `is_inline: true` 以将它们嵌入页面
